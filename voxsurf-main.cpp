...
int main(int argc, char **argv)
{
  ...
  if (env_a = std::getenv("IN_STL")) {
    if (env_b = std::getenv("OUT_VOX")) { 
      try {
        // load triangle mesh
        TriangleMesh_Ptr mesh(loadTriangleMesh(env_a));    
        ...
        // save the result
        saveAsVox(env_b, voxs);

      } catch (Fatal& e) {
        cerr << "[ERROR] " << e.message() << endl;
      }
    } else {
      cerr << "IN_STL and OUT_VOX reqd" << endl;
    }
  } else {
    cerr << "IN_STL and OUT_VOX reqd" << endl;
  }
